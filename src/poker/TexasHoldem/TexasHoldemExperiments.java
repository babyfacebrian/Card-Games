...
public class TexasHoldemExperiments {
    ...
    private static void runExp1() {
        ...
        IntStream.range(0, NUM_EXPERIMENTS).mapToObj(i -> new TexasHoldemHand.Builder()).forEach(builder -> {
            final Deck deck = Deck.shuffledDeck();
            final TexasHoldemHand hand = builder
                    .addPocketCard(deck.deal())
                    .addPocketCard(deck.deal())
    ...
    private static void runExp2() {
        ...
        IntStream.range(0, NUM_EXPERIMENTS).mapToObj(i -> new TexasHoldemHand.Builder()).forEach(builder -> {
            final Deck deck = Deck.shuffledDeck();
            final TexasHoldemHand hand = new TexasHoldemHand.Builder()
                    .addPocketCard(deck.deal())
                    .addPocketCard(deck.deal())
                    .addCommunityCard(deck.deal())
                    .addCommunityCard(deck.deal())
            ...
            final Map<Rank, List<Card>> groupByResult = hand.getHandAnalyzer().getPocketCards().stream().collect(Collectors.groupingBy(Card::getRank));
