...
import static CardsMaster.Rank.*;
...
public enum PokerHandUtils {;
    ...
    public static void checkHandClassification(final Hand hand, final ClassificationRank classificationRank){

        if(hand.getHandAnalyzer().getClassification().getClassificationRank() != classificationRank){
            throw new RuntimeException("Hand does not match expected Classification Rank ");
        }
    }

    public static Classification classifyPokerHand(final RankGroup rankGroup, final SuitGroup suitGroup, final SortedSet<Card> cards){

        final PokerHandClassifier handClassifier = new PokerHandClassifier(rankGroup, suitGroup, cards);
