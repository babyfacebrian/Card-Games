...
public class PokerHandClassifier implements HandClassifier{
    ...
    PokerHandClassifier(final RankGroup rankGroup, final SuitGroup suitGroup, final SortedSet<Card> cards) {
    ...
    private Classification isPair(){
        if(this.rankGroup.getPairCount() == 1){
            final Iterator<Map.Entry<Rank, List<Card>>> entryIterator = this.rankGroup.iterator();
            final SortedSet<Card> cards = new TreeSet<>();
            cards.addAll(entryIterator.next().getValue());
            cards.addAll(entryIterator.next().getValue());
            cards.addAll(entryIterator.next().getValue());
            cards.addAll(entryIterator.next().getValue());
    ...
    private SortedSet<Card> calculateStraight(final List<Rank> ranks){
        ...
        final Map<Rank, List<Card>> rankListMap = this.rankGroup.getRankMap();

        for(final Rank rank : ranks){
            final List<Card> values = rankListMap.get(rank);
    ...
    private Classification detectFlush(){
        final Map<Suit, List<Card>> flushSuits = this.suitGroup.getSuitGroup();

        for(final Map.Entry<Suit, List<Card>> entry : flushSuits.entrySet()){
    ...
    private Classification detectStraightFlush() {
        final Map<Suit, List<Card>> suitGroup = this.suitGroup.getSuitGroup();
